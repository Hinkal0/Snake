...
void Field::updateScales(uint32_t width, uint32_t height) {
  ...
  field.setScale({0.75f*size/600, 0.75f*size/600});
  field.setPosition({width/2.f, height/2.f+height/12.f});
  apple.setScale({0.75f*50.0f/512.0f*size/600, 0.75f*50.0f/512.0f*size/600});
  corner = {
    field.getPosition().x-field.getGlobalBounds().size.x/2+0.75f*50.0f*size/600,
    field.getPosition().y-field.getGlobalBounds().size.y/2+0.75f*50.0f*size/600
  };
  cell = 0.75f*50.0f*size/600;
  if (hasApple) {
